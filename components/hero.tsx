...
import { ArrowDownRight, ArrowRight, CalendarDays, CheckCircle2, Clock3, Code2, Gauge, Globe2, Sparkles, Zap } from "lucide-react"
    ...
    <div className="container mx-auto px-4">
      <div className="grid items-center gap-12 lg:grid-cols-[1.03fr_.97fr] lg:gap-16">
        <div className="max-w-3xl">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Button size="lg" onClick={() => scrollTo("#contact")} className="h-12 rounded-xl bg-portfolio-primary px-6 text-base text-portfolio-primary-foreground shadow-lg shadow-portfolio-primary/25 hover:bg-portfolio-primary/90">Start a project <ArrowRight className="ml-2 h-4 w-4" /></Button><a href="https://cal.com/leobislam/project-discovery" target="_blank" rel="noreferrer" className="inline-flex h-12 items-center justify-center rounded-xl border border-border bg-card/70 px-6 text-base font-medium text-foreground shadow-sm transition hover:bg-muted">Book a free call <CalendarDays className="ml-2 h-4 w-4" /></a><Button size="lg" variant="outline" onClick={() => scrollTo("#projects")} className="h-12 rounded-xl border-border bg-card/70 px-6 text-base text-foreground shadow-sm hover:bg-muted">See selected work <ArrowDownRight className="ml-2 h-4 w-4" /></Button></div>
