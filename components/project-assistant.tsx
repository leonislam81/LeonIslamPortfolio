...
export function ProjectAssistant() {
  ...
  const bookingUrl = selectedService
    ? `/book-call?project-details=${encodeURIComponent(selectedService.label)}${selectedTimeline ? `&timeline=${encodeURIComponent(selectedTimeline)}` : ''}`
    : '/book-call'
          ...
          <div className="max-h-[min(32rem,calc(100vh-11rem))] overflow-y-auto p-5">
            {!selectedService ? (
              ...
              <>
                ...
                <div className="mt-5 grid gap-2"><Link href={bookingUrl} onClick={() => trackEvent('project_assistant_booking_click', { event_category: 'engagement', service_interest: selectedService.contactService })} className="inline-flex items-center justify-center gap-2 rounded-xl bg-portfolio-primary px-4 py-3 text-sm font-semibold text-portfolio-primary-foreground transition hover:bg-portfolio-primary/90"><CalendarDays className="h-4 w-4" />Book a free call</Link><a href={whatsappUrl} target="_blank" rel="noreferrer" onClick={() => trackEvent('project_assistant_whatsapp_click', { event_category: 'engagement', service_interest: selectedService.contactService })} className="inline-flex items-center justify-center gap-2 rounded-xl border border-emerald-500/30 px-4 py-3 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-500/10 dark:text-emerald-400"><MessageCircle className="h-4 w-4" />Chat on WhatsApp</a><Link href={quoteUrl} onClick={() => trackEvent('project_assistant_quote_click', { event_category: 'engagement', service_interest: selectedService.contactService })} className="inline-flex items-center justify-center gap-2 rounded-xl border border-border px-4 py-3 text-sm font-semibold text-foreground transition hover:bg-muted"><FileText className="h-4 w-4" />Request a quote</Link></div>
