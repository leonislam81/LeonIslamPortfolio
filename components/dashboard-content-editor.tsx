...
const pageTemplates = [
  ...
  { slug: "free-audit", title: "Free audit", excerpt: "Website audit conversion page and report capture.", seo: "Lead generation" },
  { slug: "contact", title: "Contact", excerpt: "Quote and support enquiry form.", seo: "Conversion page" },
...
export function DashboardContentEditor({ initialPages, userId }: { initialPages: Page[]; userId: string }) {
  ...
  const selectPage = (slug: string) => { setSelectedSlug(slug); setNotice("") }
  ...
  return <main className="min-h-screen bg-slate-50 px-5 py-8 text-slate-950 sm:px-8"><div className="mx-auto max-w-[1440px]"><header className="flex flex-wrap items-end justify-between gap-5 border-b border-slate-200 pb-7"><div><a href="/dashboard/upcoming" className="inline-flex items-center gap-2 text-sm font-semibold text-sky-700 hover:text-sky-900"><ArrowLeft className="size-4" />Upcoming features</a><div className="mt-5 flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-xl bg-violet-100 text-violet-700"><FileText className="size-5" /></span><div><p className="text-sm font-bold uppercase tracking-[.16em] text-violet-700">Content manager</p><h1 className="mt-1 text-3xl font-bold tracking-tight">Edit public pages</h1></div></div><p className="mt-4 max-w-2xl text-sm leading-6 text-slate-600">Shape the page sections, search preview, and publishing status from one focused workspace.</p></div><div className="flex items-center gap-2"><button onClick={() => setPreview(true)} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 shadow-sm"><Eye className="size-4" />Preview</button><button onClick={() => save("Draft")} disabled={saving} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 shadow-sm"><Save className="size-4" />Save draft</button><button onClick={() => save("Published")} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-bold text-white shadow-sm">{saving ? <Loader2 className="size-4 animate-spin" /> : <FileText className="size-4" />}Publish</button></div></header>
    ...
    {preview && <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 p-4 sm:p-10"><div className="mx-auto max-w-4xl rounded-3xl bg-white p-6 shadow-2xl sm:p-10"><div className="flex items-center justify-between border-b border-slate-100 pb-5"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-sky-700">Preview · {draft.status}</p><h2 className="mt-2 text-3xl font-bold">{draft.title}</h2></div><button onClick={() => setPreview(false)} className="rounded-xl p-2 text-slate-500 hover:bg-slate-100" aria-label="Close preview"><X className="size-5" /></button></div><p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">{draft.excerpt}</p><div className="mt-8 space-y-6">{(draft.body.sections ?? []).map((section) => <div key={section.id} className="rounded-2xl bg-slate-50 p-6"><p className="text-xs font-bold uppercase tracking-[.16em] text-sky-700">{section.label}</p><h3 className="mt-2 text-2xl font-bold">{section.heading}</h3><p className="mt-3 leading-7 text-slate-600">{section.body}</p>{section.items.length > 0 && <ul className="mt-4 grid gap-2 sm:grid-cols-3">{section.items.map((item) => <li key={item} className="rounded-xl bg-white p-3 text-sm font-semibold shadow-sm">{item}</li>)}</ul>}{section.buttonLabel && <button className="mt-5 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-bold text-white">{section.buttonLabel}</button>}</div>)}</div></div></div>}
