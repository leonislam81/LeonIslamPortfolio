...
export function DashboardWorkflowSettingsForm({ initial }: { initial: DashboardWorkflowSettings }) {
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const { error } = await supabase.from("dashboard_settings").upsert({ owner_id: await getWorkspaceOwnerId(supabase, user.id), first_follow_up_days: firstFollowUpDays, re_audit_days: reAuditDays, updated_at: new Date().toISOString() })
