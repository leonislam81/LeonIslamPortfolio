...
function createAuditReportEmail(name: string, audit: AuditReport, businessGoal: AuditGoal | null) {
  ...
  const mobileChecks = audit.checks.filter((check) => mobileCheckLabels.includes(check.label))
  const otherHealthChecks = audit.checks.filter((check) => !mobileCheckLabels.includes(check.label))
  const mobileUsability = mobileChecks.length ? `<h2 style="margin:30px 0 12px;font-size:20px;">Mobile usability</h2><p style="margin:0 0 12px;color:#40536a;">These checks focus on whether visitors can read, tap, and view the page comfortably on a small screen.</p><ul style="margin:0;padding-left:20px;color:#40536a;">${mobileChecks.map((check) => `<li style="margin-bottom:8px;"><strong>${escapeHtml(check.label)}:</strong> ${check.status === "pass" ? "Looks good in this check." : escapeHtml(check.detail)}</li>`).join("")}</ul>` : ""
  const healthChecks = otherHealthChecks.length ? `<h2 style="margin:30px 0 12px;font-size:20px;">Website health checks</h2><ul style="margin:0;padding-left:20px;color:#40536a;">${otherHealthChecks.map((check) => `<li style="margin-bottom:8px;"><strong>${escapeHtml(check.label)}:</strong> ${check.status === "pass" ? "Looks good in this check." : escapeHtml(check.detail)}</li>`).join("")}</ul>` : ""
