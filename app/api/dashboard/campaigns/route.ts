...
export async function GET() {
  ...
  const { data, error } = await supabase.from("email_campaigns").select("id,subject,message,recipient_count,status,error_message,sent_at,created_at,updated_at").eq("owner_id", ownerId).order("created_at", { ascending: false }).limit(25)
...
export async function POST(request: Request) {
  ...
  if (body.action === "save") {
    const { data, error } = await supabase.from("email_campaigns").insert({ owner_id: ownerId, subject, message, recipient_count: emails.length, status: "Draft" }).select("id,subject,message,recipient_count,status,error_message,sent_at,created_at,updated_at").single()
  ...
  const html = `<div style="max-width:620px;margin:0 auto;padding:32px 20px;font-family:Arial,sans-serif;color:#10233f;line-height:1.65;">${escapeHtml(message).replace(/\n/g, "<br>")}<p style="margin-top:28px;color:#52657c;font-size:12px;">If you no longer want these updates, reply with &quot;unsubscribe&quot; and we will remove you from the list.</p></div>`
  ...
  const { data: updated } = await supabase.from("email_campaigns").update({ status: "Sent", sent_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", campaign.id).eq("owner_id", ownerId).select("id,subject,message,recipient_count,status,error_message,sent_at,created_at,updated_at").single()
