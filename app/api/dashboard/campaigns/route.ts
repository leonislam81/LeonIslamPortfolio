...
export async function GET() {
  ...
  const { data, error } = await supabase.from("email_campaigns").select("id,subject,message,recipient_count,status,error_message,sent_at,open_count,click_count,last_opened_at,last_clicked_at,delivery_status,delivered_count,bounced_count,complained_count,last_delivery_event,created_at,updated_at").eq("owner_id", ownerId).order("created_at", { ascending: false }).limit(25)
...
export async function POST(request: Request) {
  ...
  const html = `<div style="max-width:620px;margin:0 auto;padding:32px 20px;font-family:Arial,sans-serif;color:#10233f;line-height:1.65;">${renderMessage(message, campaign.id)}<p style="margin-top:28px;color:#52657c;font-size:12px;">If you no longer want these updates, <a href="https://leonislam.com/api/campaigns/unsubscribe" style="color:#0f6b8f;">unsubscribe here</a>.</p></div><img src="https://leonislam.com/api/campaigns/open?id=${campaign.id}" width="1" height="1" alt="" style="display:block;border:0;" />`
