  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <section className="relative overflow-hidden px-5 pb-14 pt-14 sm:px-8 sm:pt-20">
        ...
        <form id="audit-form" onSubmit={handleAudit} className="mx-auto mt-10 max-w-2xl rounded-2xl border border-slate-200 bg-white p-3 shadow-xl shadow-slate-900/5 sm:flex sm:flex-wrap sm:gap-3">
      ...
      {result && (
        <section aria-live="polite" className="px-5 py-14 sm:px-8 sm:py-20">
          <div className="mx-auto max-w-4xl">
            ...
            <section className="mt-8 rounded-2xl border border-sky-200 bg-sky-50 p-5"><p className="text-sm font-bold uppercase tracking-[.16em] text-sky-700">Before you re-audit</p><h3 className="mt-2 text-xl font-bold text-slate-950">Make the next comparison count</h3><div className="mt-4 grid gap-3 sm:grid-cols-3">{[["Publish the changes", "Make sure updates are live on the public URL, not only in a preview or staging site."], ["Test the visitor journey", "On a phone, check the main message, call to action, contact path, and booking flow yourself."], ["Give changes time", "After a major release, allow caches and PageSpeed data time to refresh before comparing scores."]].map(([title, detail], index) => <div key={title} className="rounded-xl bg-white p-4"><p className="text-xs font-bold uppercase tracking-wide text-sky-700">Step {index + 1}</p><p className="mt-2 font-semibold text-slate-950">{title}</p><p className="mt-1 text-sm leading-6 text-slate-600">{detail}</p></div>)}</div><a href="#audit-form" className="mt-5 inline-flex items-center gap-2 rounded-xl bg-sky-700 px-4 py-3 text-sm font-semibold text-white transition hover:bg-sky-800">Run a fresh audit <ArrowRight className="size-4" /></a></section>
