...
export default async function LeadsInboxPage() {
  ...
  const { data } = await supabase.from("audit_leads").select("id, lead_name, email, website_url, message, status, priority, created_at").order("created_at", { ascending: false }).limit(250)
  const leads = (data ?? []) as Array<{ id: string; lead_name: string | null; email: string; website_url: string; message: string | null; status: string; priority: "high" | "normal" | "low"; created_at: string }>
