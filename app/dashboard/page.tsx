...
export default async function DashboardPage() {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-6 text-slate-950 sm:px-6 lg:px-8"><div className="mx-auto max-w-[1440px]"><header className="flex flex-col gap-5 border-b border-slate-200 pb-6 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-sky-700">Leon Islam admin</p><h1 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">Business overview</h1><p className="mt-2 text-sm text-slate-600">Your visitors, enquiries, bookings, and active work in one place.</p></div><div className="flex flex-wrap gap-2"><a href="/dashboard/bookings" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:border-sky-200"><CalendarCheck2 className="size-4 text-sky-700" />Bookings</a><a href="/dashboard/settings" className="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:border-sky-200">Settings</a><DashboardSignOutButton /></div></header>{error ? <p className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-900">The database connection is active, but the dashboard tables are not ready yet. Run the supplied Supabase schema, then refresh this page.</p> : <><DashboardOnboarding /><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric icon={Users} label="Audit leads" value={leads.length} href="/dashboard/leads" /><Metric icon={CalendarCheck2} label="Confirmed bookings" value={bookingCount ?? 0} href="/dashboard/bookings" /><Metric icon={Eye} label="30-day visits" value={visitorSummary.visits} /><Metric icon={UsersRound} label="Unique visitors" value={visitorSummary.uniqueVisitors} /></div><DashboardVisitorAnalytics summary={visitorSummary} /><DashboardWorkspaceHub /><DashboardAnalytics leads={leads} /><DashboardCalendar leads={leads} /><DashboardLeadList leads={leads} /></>}</div></main>
}
